...
We can see that the between-subject SD 0.21 of the MLE method is smaller than the between-subject SD 0.26 of the REML method because MLE tends to bias the estimates towards zero.
...
::: callout-caution
The fixed effect estimates based should generally differ.
They happen to be the same here since we have a balanced random-intercept ANOVA model and certain cancellations occur so the fixed effect estimate $\hat{\beta}$ doesn't depend on how the variance components are estimated.
The algebraic details are skipped but it is straight forward to work thru.
:::
...
### Computing the Fixed Effect Estimate and its estimated standard error "by hand"
...
Hand-computed standard error (this is simply the GLS covariance, also see Section 5.1 in @bates_fitting_2015 for how this is computed):
...
This subject does not focus on model diagnostics, but the general idea behind these plots is to evaluate whether our model assumptions are met.
...
We fit models with both random intercept and random slope (person growth rate varies) using both the REML (default) and the MLE (`REML = FALSE`) estimation methods:
...
Finally we can access the standard errors of the fixed effects:
...
Since income is on a log-scale, this values is multiplicative---95% of incomes are between $\exp(-1.96\sigma_{b_0})$= `r {round(exp(-1.96*as_tibble(s_psid_reml$varcor)$sdcor[1]), 2)}` and $\exp(1.96\sigma_{b_0})$= `r {round(exp(1.96*as_tibble(s_psid_reml$varcor)$sdcor[1]), 2)}` times the overall mean income in 1978.

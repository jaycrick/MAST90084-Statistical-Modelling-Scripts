...
summary(happy_logit1_vglm) # the coefficient values are  same as
# those reported in the first six row of Table 3.11 in F & T.

attributes(happy_logit1_vglm)$predictors # we can also look at the predictors
sqrt(diag(vcov(happy_logit1_vglm))) # same as the std. error reported by summary(happy_logit1_vglm )
...
happy_logit2_vglm_coeff # For now, only the two intercept estimates, which correspond to reference level <12 of School, matches the numbers in the book
...
## a = attributes(summary(happy_logit1_vglm ))$criterion$loglikelihood+
## attributes(summary(happy_logit2_vglm ))$criterion$loglikelihood

# These get the log likelihood of individual groups
logLik(happy_logit1_vglm, sum = F)
logLik(happy_logit2_vglm, sum = F)

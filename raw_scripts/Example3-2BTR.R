...
library(MASS)
BTR.dat <- read.csv("Example3-2BTR.csv", colClasses = c("factor", "factor", "factor", "numeric"))  
str(BTR.dat)
...
BTR.logit=polr(BTR~Age+Smoking+Age:Smoking, data=BTR.dat, 
               weights=Freq, Hess=T,method="logistic")
summary(BTR.logit)
...
BTR.logit1=polr(BTR~Age+Smoking, data=BTR.dat,
                weights=Freq, Hess=T,method="logistic")
summary(BTR.logit1)
...
BTR.ph=polr(BTR~Age+Smoking+Age:Smoking,data=BTR.dat, 
            weights=Freq, Hess=T, method="cloglog")
summary(BTR.ph)

######Fit a cumulative model with the extreme maximal-value distribution:################
BTR.extm=polr(BTR~Age+Smoking+Age:Smoking,data=BTR.dat,
              weights=Freq, Hess=T,method="loglog")
summary(BTR.extm)
...
for (row in 1:nrow(data_mat)){
  ...
  eval_vec = rep(0, 3)
  eval_vec[col] = 1
  increment =data_mat[row, col]*dmultinom(x = eval_vec, prob = probs[row, ]  , log = TRUE)
  print(increment)
  if (!is.na(increment))
...
summary( BTR.logit)$coefficients[, 1] + qnorm(0.975)* sqrt(diag(vcov(BTR.logit) ))  # not the same as confint(BTR.logit)
summary( BTR.logit)$coefficients[, 1] + qnorm(0.025)* sqrt(diag(vcov(BTR.logit) ))  # not the same as confint(BTR.logit)

...
glmer_fit = lme4::glmer(resp ~  age*smoke     +
                          (1|id), data = ohio, family = binomial,
...
glmer_fit_nAGQ_10 = lme4::glmer(resp ~  age*smoke     +
                          (1|id), data = ohio, family = binomial,
                        control = glmerControl(optimizer ="Nelder_Mead"),
                        nAGQ = 10)


summary(glmer_fit_nAGQ_10 )
...
glmmPQL_fit = MASS::glmmPQL(resp ~  age*smoke  , random = ~ 1 | id,
        family = binomial, data = ohio)   #based on penalzied quasi-likelihood

summary(glmmPQL_fit)

head(ranef(glmmPQL_fit)  )# this should be the version of ranef() in nlme
...
wine_fixed_fit_polr =  polr(rating~temp + contact, data=wine, Hess=T,method="logistic")
# use our old polr() function for comparison
summary(wine_fixed_fit_polr  )


#  we will also try the function in the ordinal package
...
wine_mixed_fit <-  clmm(rating ~ temp + contact + (1|judge), data = wine, nAGQ = 10)

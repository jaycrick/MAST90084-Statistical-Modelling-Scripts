...
caesar_tibble <- dplyr::tbl_df(caesar)    # create a "tibble" dataframe

caesar_grouped <- dplyr::group_by(caesar_tibble, noplan, factor, antib)  # Group the data by different combinations of the covariates

caesar_summary <- dplyr::summarize(caesar_grouped, size = sum(w), Infect_count = sum(yl*w) )
...
row.names(caesar) = 1:nrow(caesar)
caesar
...
# fit the regression
caesar.glm <- glm(Infect_count/size ~ antib +factor +  noplan, data=caesar , weight=size,
                  family=binomial)  # default link is logit already
...
caesar.glm # the coeff estimates are again same as the numbers in example 2.1 
summary(caesar.glm)
deviance(caesar.glm) 
...
anova(caesar.glm, test = "Chisq")
anova(caesar.glm, test = "F")  # complain that F test isn't suitable here!

resid(caesar.glm, type = "deviance")
resid(caesar.glm, type = "pearson")
resid(caesar.glm, type = "working")  ## caesar.glm$residuals will also give you the working residuals!
resid(caesar.glm, type = "response")
sum (resid(caesar.glm, type = "deviance")^2)  # equal to caesar.glm$deviance 

## Pearson and deviance test for model goodness-of-fit
1- pchisq(caesar.glm$deviance, df = 3)
1 - pchisq(sum(resid(caesar.glm, type = "pearson")^2), df = 3)
...
round((caesar.glm)$fitted.values, digits = 2)  # fitted
...
caesar.glm.probit <- glm(Infect_count/size ~ antib +factor +  noplan, data=caesar , weight=size,
                  family=binomial(link = "probit"))
summary(caesar.glm.probit)$coeff # estimates same as those reported by the book.
...
caesar.glm.noplan.antib <- glm(Infect_count/size ~ factor +  noplan*antib , data=caesar , weight=size,
                  family=binomial)  

anova(caesar.glm.noplan.antib)  # get the same residual deviances as reported in the book

noplan_antib_interaction_delta_deviance <- anova(caesar.glm.noplan.antib)$Deviance[5]  
...
caesar.glm.noplan.factor <- glm(Infect_count/size ~  noplan + factor + antib + noplan:factor, 
                   data=caesar , weight=size, family=binomial)  

# eqiuvalently, you can do
# caesar.glm.noplan.factor <- glm(Infect_count/size ~  antib + noplan*factor, 
#                    data=caesar , weight=size, family=binomial)  


summary(caesar.glm.noplan.factor )  # will see some enormous estimates and std.error for some coefficients

1 - pchisq(   sum(resid(caesar.glm.noplan.factor,type="pearson")^2), df = 2     )
1 - pchisq(   sum(resid(caesar.glm.noplan.factor,type="deviance")^2), df = 2     )
...
round(caesar.glm.noplan.factor$fitted.values, 2)  # fitted
...
caesar.glm.noplan.factor.augment <- glm(Infect_count/size ~  noplan + factor + antib + noplan:factor , 
                   data=caesar_augment , weight=size, family=binomial)  


summary(caesar.glm.noplan.factor.augment )  

# actual vs fitted
round(
rbind(caesar_augment$Infect_count/caesar_augment$size,  
caesar.glm.noplan.factor.augment$fitted)
, digits = 2)

# chi-square tests
1 - pchisq(   sum(resid(caesar.glm.noplan.factor.augment,type="pearson")^2), df = 2     )
1 - pchisq(   sum(resid(caesar.glm.noplan.factor.augment,type="deviance")^2), df = 2     )
# do not reject the new model with noplan.factor interaction at usual level such as 0.1 or 0.05

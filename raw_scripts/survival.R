...
plot(log(cancer.km$time), log(-log(cancer.km$surv)), 
     type = "n", xlab = "Log(time)", ylab = "Log(-Log(S(time)))")
...
plot(cancer.km, fun = "cloglog", lty = 1:2, col = c("Red", "Blue"), xlim = c(5, 300))
...
# (a)
cancer.exp <- survreg(Surv(time, status) ~ group, data = bcancer, dist = "exponential")
summary(cancer.exp)

# wald test
wald_stat <- cancer.exp$coeff[2]/sqrt(cancer.exp$var[2, 2])
2*pnorm( - abs( wald_stat)) # p-value

# LR test
lr_stat <- -2*(cancer.exp$loglik[1] -  cancer.exp$loglik[2])
pchisq(q = lr_stat, df = 1, lower.tail = FALSE)  # p-value

# (b) 
exp(-cancer.exp$coef[1])
#(Intercept) 
#0.003026634
exp(-sum(cancer.exp$coef))  
#[1] 0.007838746


# (c)
sum(bcancer$status[bcancer$group=="N"])/sum(bcancer$time[bcancer$group=="N"])  # MLE of lambda for group "N" using the formula
#[1] 0.003026634
sum(bcancer$status[bcancer$group=="P"])/sum(bcancer$time[bcancer$group=="P"]) # MLE of lambda  for group "P" using the formula
#[1] 0.007838746

## (d)  compute CI using delta method
beta = cancer.exp$coeff[2]
var_beta = (cancer.exp$var)[2, 2]
...
#######################
# weibull regression
cancer.wbl <- survreg(Surv(time, status) ~ group, data = bcancer, dist = "weibull")
summary(cancer.wbl)
cancer.wbl$coef
cancer.wbl$scale
cancer.wbl$var
...
#(a)
lambda_n = exp(-cancer.wbl$coef[1])

lambda_p = exp(-sum(cancer.wbl$coef))

alpha = 1/cancer.wbl$scale
...
## Wald
log(cancer.wbl$scale)/sqrt(cancer.wbl$var[3,3])
  
#The p-value is 
2*pnorm(- abs(log(cancer.wbl$scale)/sqrt(cancer.wbl$var[3,3])) )  

## LR
2*(cancer.wbl$loglik[2] - cancer.exp$loglik[2])

qchisq(.95,  df = 1)
pchisq(2*(cancer.wbl$loglik[2] - cancer.exp$loglik[2]),  df = 1, lower.tail = FALSE)